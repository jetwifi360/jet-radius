...
function Logout() {
    // Clear all cookies using document.cookie for reliability
    var cookies = document.cookie.split(";");
    for (var i = 0; i < cookies.length; i++) {
        var cookie = cookies[i];
        var eqPos = cookie.indexOf("=");
        var name = eqPos > -1 ? cookie.substr(0, eqPos).trim() : cookie.trim();
        if (name.startsWith('BSK_')) {
            document.cookie = name + "=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/";
        }
    }
    // Also clear using jQuery if available
            ...
            success: function (data) {
                ...
                $.each(data.data, function (i, menu) {
                    ...
                    if (hasChildren) {
                        ...
                        html += '<a class="nav-link" href="' + menu.value + '.html">';
                    }
                    ...
                    if (hasChildren) {
                        ...
                        $.each(menu.children, function (index, subs) {
                            var hasGrand = (subs.children && subs.children.length);
                            html += '<li class="nav-item">';
                            if (hasGrand) {
                                html += '<a class="nav-link" href="javascript:void(0);" data-toggle="collapse" data-target="#page_' + subs.id + '">' + subs.name + '</a>';
                                html += '<ul id="page_' + subs.id + '" class="nav flex-column collapse collapse-level-1">';
                                $.each(subs.children, function (j, subs2) {
                                    html += '<li class="nav-item">';
                                    html += '<a class="nav-link" href="' + subs2.value + '.html">' + subs2.name + '</a>';
                                    html += '</li>';
                                });
                                html += '</ul>';
                            } else {
...
$.ajax({
    url: "./api/index.php?pages=profile",
    headers: {
        "Api": $.cookie("BSK_API"),
        "Key": $.cookie("BSK_KEY"),
        "Accept": "application/json"
...
$('form[id^=import-]').each(function (index, element) {
    ...
    $('#import-' + id).validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: "./api/index.php?pages=" + action,
                headers: {
                    "Api": $.cookie("BSK_API"),
                    "Key": $.cookie("BSK_KEY"),
                    "Accept": "application/json"
