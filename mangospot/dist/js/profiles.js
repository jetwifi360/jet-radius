...
function Action() {
        ...
        $.ajax({
            ...
            success: function (detail) {
                if (detail.status) {
                    ...
                    $('#profile_id').val(detail.data.groupname); // Set profile_id for edit
                    ...
                    $.ajax({
                        ...
                        success: function (mt) {
                            if (mt.status) {
                                var hp = mt.data.hotspot_profiles || [];
                                var pools = mt.data.address_pools || [];
                                var $hp = $('#mt_hotspot_profile');
                                var $pool = $('#mt_address_pool');
                                $hp.empty().append('<option value="">Select server profile</option>');
                                $pool.empty().append('<option value="">Select address pool</option>');
                                hp.forEach(function(n){ $hp.append('<option value="'+n+'">'+n+'</option>'); });
                                pools.forEach(function(n){ $pool.append('<option value="'+n+'">'+n+'</option>'); });
                            } else {
                                $('#mt_hotspot_profile').empty().append('<option value="">Unavailable</option>');
                                $('#mt_address_pool').empty().append('<option value="">Unavailable</option>');
                            }
                        }
                    });
                }
    ...
    $('#form-data').on('submit', function(){
        var hpVal = $('#mt_hotspot_profile').val() || '';
        var poolVal = $('#mt_address_pool').val() || '';
        if (hpVal) {
            $('<input>').attr({type:'hidden', name:'radgroupreply[]'}).val(hpVal).appendTo('#form-data');
            $('<input>').attr({type:'hidden', name:'attribut[]'}).val('Mikrotik-Hotspot-Profile').appendTo('#form-data');
        }
        if (poolVal) {
            $('<input>').attr({type:'hidden', name:'radgroupreply[]'}).val(poolVal).appendTo('#form-data');
            $('<input>').attr({type:'hidden', name:'attribut[]'}).val('Framed-Pool').appendTo('#form-data');
        }
    });
};
