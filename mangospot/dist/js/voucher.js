...
function Action() {
    ...
    $('#card-scale, #page-padding, #grid-gap').on('input change', function(){
        var cols = parseInt($('#cards-per-row').val()) || 3;
        if (Array.isArray(window.__printItems) && window.__printItems.length) {
            applyGridStyles(cols);
            var grid = buildGrid(window.__printItems, cols);
...
function getNumberInput(id, def){
...
function fitCards(){
    ...
    cells.forEach(function(cell){
        ...
        if(w && h){
            var scalePct = getNumberInput('card-scale', 105);
            var s = cw / w * (scalePct/100);
            var sh = h * s;
