...
if(isset($_POST['qty'])){
    ...
    // Include pool mapping configuration
    require_once 'mikrotik_pool_mapping.php';
    
    // Get Mikrotik hotspot profile and pool mapping from radgroupreply
    $hp = $Bsk->Show("radgroupreply", "value", "groupname='$profil' AND attribute='Mikrotik-Hotspot-Profile'");
    $pool = $Bsk->Show("radgroupreply", "value", "groupname='$profil' AND attribute='Framed-Pool'");
    $group = $Bsk->Show("radgroupreply", "value", "groupname='$profil' AND attribute='Mikrotik-Group'");
    
    if($pool && !empty($pool['value'])){
        $pool_name = $pool['value'];
    }
    if($group && !empty($group['value'])){
        $mikrotik_group = $group['value'];
    }
    
    // Map hotspot profiles to their corresponding NAS IP addresses and pools
    if(!$nas_ip || empty($pool_name) || empty($mikrotik_group)){
        $pool_config = null;
        
        // Try to get pool config from hotspot profile first
        if($hp && !empty($hp['value'])){
            $pool_config = get_pool_config_by_profile($hp['value']);
        }
        
        // Fallback to profile name if hotspot profile not found
        if(!$pool_config){
            $pool_config = get_pool_config_by_profile($profil);
        }
        
        // Apply configuration if found
        if($pool_config){
            if(empty($nas_ip)) $nas_ip = $pool_config['nas_ip'];
            if(empty($pool_name)) $pool_name = $pool_config['pool_name'];
            if(empty($mikrotik_group)) $mikrotik_group = $pool_config['mikrotik_group'];
        }
    }
    
    // Final fallback to ensure we have pool configuration
    if(empty($pool_name) || empty($nas_ip)){
        if($nas_ip){
            $pool_config = get_pool_config_by_nas($nas_ip);
            if($pool_config){
                if(empty($pool_name)) $pool_name = $pool_config['pool_name'];
                if(empty($mikrotik_group)) $mikrotik_group = $pool_config['mikrotik_group'];
            }
    ...
    for($i=1; $i<=$qty; $i++){
        ...
        $batch_pswd = ($pass_len > 0 ? random_str($pass_len, $charec) : '');
        $mode_paswd = ($mode == 'true' ? $batch_pswd : ($mode == 'username_only' ? '' : $batch_user));
        ...
        try {
            $inserted = true;
            if($pass_len > 0 && $mode !== 'username_only'){
                $add_voucher = $Bsk->Insert("radcheck", $post_array);
                $inserted = ($add_voucher ? true : false);
            }
            ...
            if($inserted){
                ...
                // Assign IP pool directly to user if specified
                if($pool_name){
                    $Bsk->Delete("radreply", array("username"=>$batch_user, "attribute"=>"Framed-Pool"));
                    $Bsk->Insert("radreply", array(
                        "identity"   => $identity_id,
                        "users"      => $user_id,
                        "username"   => $batch_user,
                ...
                // Assign Mikrotik group if specified
                if($mikrotik_group){
                    $Bsk->Delete("radreply", array("username"=>$batch_user, "attribute"=>"Mikrotik-Group"));
                    $Bsk->Insert("radreply", array(
                        "identity"   => $identity_id,
                        "users"      => $user_id,
                        "username"   => $batch_user,
        ...
        foreach($users as $value_print){
            ...
            if($query_theme){
                ...
                if($bg){
                    ...
                    $bgStyle = 'background:url('.$bg.') no-repeat center/contain;';
                } else {
                    $bgStyle = '';
                }
                ...
                foreach($layout as $it){
                    ...
                    $style = 'position:absolute;left:'.intval($it['x']).'px;top:'.intval($it['y']).'px;color:'.$it['color'].';font:'.($it['bold']?'bold ':'').intval($it['size']).'px '.$it['family'].';';
