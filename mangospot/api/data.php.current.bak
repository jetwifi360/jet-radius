...
if(isset($_GET['menu'])){
    ...
    foreach($query_menu as $link_menu){
        $tree_menu[] = $link_menu;
        if(!$master_id && (strtolower($link_menu['name']) == 'master' || strtolower($link_menu['value']) == 'master')){
            $master_id = $link_menu['id'];
        }
    }
    if($master_id){
        $card_id = 99999; // synthetic id for Card Designer group
        $tree_menu[] = array('id'=>$card_id,'name'=>'Card Designer','value'=>'card_designer','icon'=>'style','slug'=>$master_id);
        $tree_menu[] = array('id'=>99998,'name'=>'Html Theme Designer','value'=>'themes','icon'=>'code','slug'=>$card_id);
        $tree_menu[] = array('id'=>99997,'name'=>'Drag and Drop Designer','value'=>'designer','icon'=>'edit','slug'=>$card_id);
    }
    $tree = Tree($tree_menu);
...
if(isset($_GET['accept'])){
    $menu_access = array();
    if(!empty($Menu["value"])) { $query_access = $Bsk->Select("menu", "value", "id in ($Menu[value])"); } else { $query_access = array(); }
    ...
    echo json_encode($menu_access ?
        array(
            ...
            "message"	=> "success", 
            "color"		=> "green",
            "data"		=> array_merge($menu_access, array('Profile','User','Designer',''))
        ) :
        array(
            "status"	=> false, 
            "message"	=> "error", 
