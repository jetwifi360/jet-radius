...
if(isset($_GET['data'])){
    ...
    $setPPP = ($setType ? array("?service" => $setType) : array());
    $setHSP = ($setType ? ($setType == 1 ? array() : array("?address" => $setType)) : array());
        ...
        if ($Router->connect($trafic['nasname'].$ports, $trafic['username'], $Auth->decrypt($trafic['password'], 'BSK-RAHMAD'))) {
            ...
            foreach ($hspArr as $hspList) {
                $online[] = array(
                    ...
                    "timeleft"  => (isset($hspList['session-time-left']) ? $hspList['session-time-left'] : ""),
                    "id"        => $hspList['.id']
                );
            }
            foreach ($pppArr as $pppList) {
                $online[] = array(
                    ...
                    "timeleft"  => (isset($pppList['session-time-left']) ? $pppList['session-time-left'] : ""),
                    "id"        => $pppList['.id']
                );
            }
        }
