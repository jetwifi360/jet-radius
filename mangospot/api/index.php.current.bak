...
$Default    = array("login", "forgot", "data", "profile", "voucher", "designer", "themes");
$inMenu     = ($Menu ? explode(',', $Menu['value']) : array());
$Access     = $Bsk->Show("menu", "*", "value = '$Page' and slug != 0 and status = 'true'");
// file_put_contents('/tmp/debug_api.txt', "Page: [$Page]\nAccess: " . ($Access ? 'Found' : 'False') . "\n", FILE_APPEND);
$Include    = (in_array($Page, $Default) ? $Page : (($Access && in_array($Access['id'], $inMenu)) ? $Access['value'] : false));
