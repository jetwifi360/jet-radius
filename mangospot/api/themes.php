...
$Bsk      = new Connect();
$Bsk->setAttribute(PDO::ATTR_EMULATE_PREPARES, true);
...
if(isset($_POST['name'])){
    try {
        ...
        $ok = ($check_post ? 
            $Bsk->Change("themes", $payload_update, "id = '$check_post[id]' ") : 
            $Bsk->Insert("themes", array_merge($payload_update, array("identity" => $Menu['identity'], "users" => $Menu['id'])))
        );
